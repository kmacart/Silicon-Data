...
public class GameBoard
{
   ...
   Scene sceneSetup()
   {
	   ...
	   Label[] playerNames =
	      new Label[gameControl.getGameRules().getNumberOfPlayers()];
	   ...
	   for(int i = 0; i < gameControl.getGameRules().getNumberOfPlayers(); i++)
	   {
		   ...
		   playerNames[i].setBackground(new Background(new
              BackgroundFill(players[i].getColour(), CornerRadii.EMPTY,
              Insets.EMPTY)));
	   ...
	   scene.getStylesheets().add("data/GameBoard.css");
   ...
   VBox scoreDisplay()
   {
	   ...
	   Player[] players = gameControl.getPlayers();
	   playerScores = new
	      Label[gameControl.getGameRules().getNumberOfPlayers()];
	   playerNames = new Label[gameControl.getGameRules().getNumberOfPlayers()];
	   
	   round = new Label("Round: " +
	      gameControl.getGameState().getGameRound());
   ...
   void plotCard(ImageView cardView, Location location)
   {
	   ...
	   if (!location.getHorizontal()) cardView.setRotate(270);
   ...
   void redrawAllCards()
   {
	   ...
	   for(int i = 0; i < deck.length; i++)
	   {
		   ...
		   if (!location.getHorizontal()) cardViews[i].setRotate(270);
		   cardViews[i].setX(location.getXCoord());
		   cardViews[i].setY(location.getYCoord());
		   group.getChildren().add(cardViews[i]);
	   }
	   if(gameControl.getGameState().getDeckPointer() <
	      gameControl.getGameRules().getNumberOfCards())
	   {
		   selectedCard = gameControl.getGameState().getDeckPointer();
    	  ...
    	  selectedCardView.setOnMouseClicked(e ->
    	  {
    		  if(e.getButton().equals(MouseButton.PRIMARY))
    		  {
    			 ...
    			 if(!selectToggle)
    			 {
    				...
    				removeEventHandlers();
    			    PlayerMove move = new PlayerMove("Buy Card",
    			       deck[selectedCard], 10);
    				   gameControl.updateGameState(move);
    			 }
	  ...
	  } else
	  {
		 ...
		 cardPane.setOnMouseMoved(event ->
		 {
		    ...
		    if (selectedCardLocation.getHorizontal())
		    {
		    	...
		    	if(xVal >= 540.0) xVal = 540.0;
			    selectedCardView.setX(xVal);
			    if (yVal <= 0.0) yVal = 0.0;
			    if(yVal >= 576.0) yVal = 576.0;
		        selectedCardView.setY(yVal);
		     } else
		     {
		        if(xVal <= 0.0 - cardVertXOffset) xVal = 0.0 - cardVertXOffset;
				if(xVal >= 576.0 - cardVertXOffset) xVal = 576.0 - cardVertXOffset;
			    selectedCardView.setX(xVal);
		        if(yVal <= 0.0 + cardVertYOffset) yVal = 0.0 + cardVertYOffset;
				if(yVal >= 540.0 + cardVertYOffset) yVal = 540.0 + cardVertYOffset;
			    selectedCardView.setY(yVal);
		     }
		  });
	     selectToggle = true;
 	   ...
 	   for(Location location: locations)
 	   {
 		   ...
 		   if(cardRect.intersects(cardZone))
 		   {
 			   for(Location otherLocation: locations)
 			   {
 				   ...
 				   if(cardRect.intersects(otherCard)) return false;
 			   }
	   ...
	   for(Location location: locations)
	   {
		   ...
		   if(xCoord <= 0.0)
		   {
			   rectangle = new Rectangle(0.0,
					      yCoord - cardLayoutMargin, width + cardLayoutMargin,
					      height + cardLayoutMargin * 2.0);
		   } else if(xCoord + width >= 648.0)
		   {
			   rectangle = new Rectangle(xCoord - cardLayoutMargin,
					      yCoord - cardLayoutMargin, width + cardLayoutMargin,
					      height + cardLayoutMargin * 2.0);
		   } else if(yCoord <= 0.0)
		   {
			   rectangle = new Rectangle(xCoord - cardLayoutMargin,
			      0.0, width + cardLayoutMargin * 2.0,
			      height + cardLayoutMargin);
		   } else if(yCoord + height >= 648.0)
		   {
			   rectangle = new Rectangle(xCoord - cardLayoutMargin,
			      yCoord - cardLayoutMargin, width + cardLayoutMargin * 2.0,
				  height + cardLayoutMargin);
		   } else
		   {
			   rectangle = new Rectangle(xCoord - cardLayoutMargin,
					      yCoord - cardLayoutMargin,
					      width + cardLayoutMargin * 2.0,
					      height + cardLayoutMargin * 2.0);
		   }
   ...
   Rectangle2D getRect(Location location)
   {
	   ...
	   Rectangle2D rectangle = new Rectangle2D(xCoord, yCoord,
			      width, height);
   ...
   void cardOutlines(Player player)
   {
      ...
      Color playerColour = player.getColour();
 	  ...
 	  for(Location location: locations)
 	  {
 		   ...
 		   Rectangle rectangle = new Rectangle(xCoord, yCoord,
 				      width, height);
   ...
   {
	   drawScores();
	   gameControl.newLogEntry("Game won in " +
	      gameControl.getGameState().getGameRound() + " rounds.");
	   buyCard.setDisable(true);
	   attackCard.setDisable(true);
	   buyResearch.setDisable(true);
	   saveButton.setDisable(true);
