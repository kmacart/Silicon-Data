...
public class GameControl
{
   ...
   Card[] placePlayerCards(Card[] deck)
   {
	   ...
	   Card[] remainder = new Card[gameRules.getNumberOfCards() -
	      newOrderNumber];
	   ...
	   for(Card card: deck)
	   {
		   if(card.getOwner() == null)
		   {
			   ...
			   Location location = new Location(gameRules.getMainDeckX(),
			      gameRules.getMainDeckY(), gameRules.getHorizontal());
   ...
   void updateGameState(PlayerMove playerMove)
   {
	   ...
	   } else
	   {
		   ...
		   newLogEntry(player.getName() + " converted " + cost +
		      " gold pieces into research.");
		   // Need to check for victory here
		   if(player.getResearch() >= (player.getModuleLevel() + 1) * 100)
		   {
		      newLogEntry(player.getName() +
		      " has upgraded to a new level.");
	   ...
	   } else
	   {
		   ...
		   if((players[playerTurn].getMoney() < 10)
		      && (!attack.canAttack())
		      && (players[playerTurn].getMoney() <
		      players[playerTurn].getNewResearchCost()))
		   {
			   newLogEntry(players[playerTurn].getName() + " cannot make a" +
		          " move - action moves to next player.");
			   nextPlayer();
			   giveEarnings();
			   gameBoard.drawScores();
		   }
   ...
   void addResearch()
   {
	   ...
	   for(int i = 0; i < gameRules.getNumberOfPlayers(); i++)
	   {
		   ...
		   if(players[i].getResearch() >=
		      (players[i].getModuleLevel() + 1) * 100)
		   {
			   newLogEntry(players[i].getName() +
			      " has upgraded to a new level.");
   ...
   void printScores()
   {
	   ...
	   System.out.println("Scores after round " + gameState.getGameRound() +
	      " are as follows:");
	   for(int i = 0; i < gameRules.getNumberOfPlayers(); i++)
	   {
		   System.out.println(players[i].getName() + " gold: " +
	          players[i].getMoney() + " research: " + players[i].getResearch());
	   }
   }
   ...
   void giveEarnings()
   {
	   ...
	   if(!player.hasCredentials())
	   {
	      for(Card card: playerCards)
		  {
		     if(card.getName().equals("Joker1") ||
			    card.getName().equals("Joker2"))
			    {
				   earnings -= 10;
			    }
		  }
	   ...
	   newLogEntry(player.getName() + " earned " + earnings +
	      " gold pieces from assets last round.");
   }
   ...
   {
	   return players[playerTurn].getMoney() >=
	      players[playerTurn].getNewResearchCost();
   }
   ...
   void upgradeAbilities(Player player)
   {
	   ...
	   {
		   player.setMindSlaves(true);
		   newLogEntry(player.getName() + " now has the social media" +
		      " mind slaves ability.");
	   }
	   if(level > 1 && !player.hasSurveillance())
	   {
		   player.setSurveillance(true);
		   newLogEntry(player.getName() + " now has the all seeing" +
		      " eye surveillance ability.");
	   }
	   if(level > 2 && !player.hasCredentials())
	   {
		   player.setCredentials(true);
		   newLogEntry(player.getName() + " now has the deep state" +
		      " credentials ability.");
	   }
	   if(level > 3 && !player.hasDroneMercenaries())
	   {
		   player.setDroneMercenaries(true);
		   newLogEntry(player.getName() + " now has the self-aware" +
		      " drone mercenaries ability.");
	   }
   }
}
